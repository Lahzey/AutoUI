...
public class VariableExpression : Expression
{
    ...
    public override Type GetExpectedType()
    {
        ...
        else if (contextType == typeof(object)) return typeof(object);
        else return contextType.GetField(VariableName)?.FieldType ?? contextType.GetProperty(VariableName)?.PropertyType ?? typeof(object);
    }
    ...
    public bool? IsValid()
    {
        ...
        else if (contextType == typeof(object)) return null; // context type object means unknown, so we cannot know if the variable is valid
        else return contextType.GetField(VariableName) != null || contextType.GetProperty(VariableName) != null;
    }
...
[ExpressionPattern(typeof(Expression), @" \. (", typeof(VariableExpression), @"(?: \. ", typeof(VariableExpression), ")*)", AutoSpace = false), PatternPriority(1)]
